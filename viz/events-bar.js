...
export function renderEventsBar(el, step) {
  ...
  const left = 40, base = h - 40, top = 28;
  const colW = (w - left - 12) / years.length;
  const active = step === 0 ? HAZARDS : HAZARDS.filter(t => hazardFilter.has(t));
  const cells = [];
  years.forEach((y, yi) => {
    let k = 0;
    active.forEach(type => {
  ...
  const barUnit = (base - top) / maxStack;
  const geom = c => ({
    x: left + c.yi * colW, y: base - (c.idx + 1) * barUnit, w: Math.max(3, colW - 3), h: barUnit + .6, r: 0
  });
  const fill = c => step === 0 ? C.turq : HAZ_COLOR[c.type];

  svg.selectAll("rect.blank-catcher").data(step >= 1 ? [0] : []).join("rect")
  ...
  g.selectAll("rect").data(cells, d => d.key).join(
    ...
    update => update.call(s => s.transition(T()).delay(c => c.yi * 6).attr("x", c => geom(c).x).attr("y", c => geom(c).y).attr("width", c => geom(c).w).attr("height", c => geom(c).h).attr("rx", c => geom(c).r).attr("fill", fill).attr("opacity", .94)),
    ...
    PLOTS.events(el, state.events ?? 0);
  }).on("mousemove", (ev, c) => showTip(ev, step === 1 ? `<b>${c.y} · ${c.type}</b><br>${cells.filter(x => x.y === c.y && x.type === c.type).length} ${c.type.toLowerCase()} disasters` : `<b>${c.y}</b><br>${cells.filter(x => x.y === c.y).length} disasters`)).on("mouseleave", hideTip);
  ...
  svg.selectAll("text.unit").data([0]).join("text").attr("class", "unit").attr("x", left).attr("y", 14).attr("font-size", 15).attr("fill", C.mist).text(step === 0 ? "Number of disasters (2000-2026)" : "Number of disasters by types (2000-2026)");
  ...
  svg.selectAll("text.axis-y").data([0]).join("text").attr("class", "axis-y").attr("transform", `translate(13,${top + (base - top) / 2}) rotate(-90)`).attr("text-anchor", "middle").attr("font-size", 11.5).attr("fill", C.mist).text("Number of events");
}
